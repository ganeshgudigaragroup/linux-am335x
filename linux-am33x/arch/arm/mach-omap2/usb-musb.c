...
void __init usb_musb_init(struct omap_musb_board_data *musb_board_data,
                          struct omap_musb_board_data *musb_second_data)
{
	...
	musb_plat[0].extvbus = board_data->extvbus;
        ...
        if(musb_second_data != NULL) {

        musb_plat[1].clock = "ick";
        musb_plat[1].board_data = musb_second_data;
        musb_plat[1].power =musb_second_data->power >> 1;
        musb_plat[1].mode = musb_second_data->mode;
        musb_plat[1].extvbus = musb_second_data->extvbus;
        }
        
         printk(KERN_INFO"%d ext vbus \n\r",musb_plat[0].extvbus);
